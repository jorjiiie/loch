...
type prim1 = Add1 | Sub1 | Print | IsBool | IsNum | IsTuple | Not | PrintStack | Thread | Get | Start
...
and call_type = Native of string | Snake | Prim | Unknown
...
let map_opt f v = match v with None -> None | Some v -> Some (f v)
...
let get_tag_D d = match d with DFun (_, _, _, t) -> t
