...
let string_of_call_type ct =
  match ct with Native _ -> "*" | Snake -> "" | Prim -> "#" | Unknown -> "?"

let rec string_of_binding_with (depth : int) (print_a : 'a -> string)
...
let string_of_program_with (depth : int) (print_a : 'a -> string)
    (p : 'a program) : string =
  match p with
...
let rec string_of_aexpr_with (depth : int) (print_a : 'a -> string)
    (e : 'a aexpr) : string =
...
and string_of_cexpr_with (depth : int) (print_a : 'a -> string) (c : 'a cexpr) :
  ...
  if depth <= 0 then "..."
    ...
    | CPrim2 (op, left, right, a) ->
        sprintf "(%s %s %s)%s" (string_of_immexpr left) (string_of_op2 op)
...
let rec format_bind (fmt : Format.formatter) (print_a : 'a -> string)
    (b : 'a bind) : unit =
  match b with
  | BBlank a ->
...
let rec format_expr (fmt : Format.formatter) (print_a : 'a -> string)
    (e : 'a expr) : unit =
...
let format_declgroup (fmt : Format.formatter) (print_a : 'a -> string)
    (d : 'a decl list) : unit =
  print_list fmt
