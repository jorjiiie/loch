...
exception
  ShadowId of
    string * sourcespan * sourcespan (* name, where used, where defined *)
...
exception
  Arity of
    int * int * sourcespan (* intended arity, actual arity, where called *)
...
exception
  LetRecNonFunction of
    sourcespan bind * sourcespan (* name binding, where defined *)

exception
  ShouldBeFunction of string * sourcespan (* name, where defined, actual typ *)
...
exception
  DeclArity of
    ...
    * int
    * int
    * sourcespan (* name, num args, num types, where defined *)
  ...
  List.map
    (fun e ->
      ...
      | LetRecNonFunction (bind, loc) ->
          sprintf
            "Binding error at %s: Let-rec expected a name binding to a lambda; \
             got %s"
