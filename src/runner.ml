...
let test_does_run filename test_ctxt =
  ...
  runner ~no_builtins:opts.no_builtins ~args:args ~std_input:input prog ("do_pass/" ^ filename) output test_ctxt
    ~cmp: (fun check result ->
      match check, result with
...
let input_file_test_suite () =
  let safe_readdir dir ext =
    ...
    with _ -> [] in
  "input-file-suite">:::[
      "do_pass"  >:::(List.map (fun f -> f>::test_does_run f)   (safe_readdir "input/do_pass"   ".loch"));
      "do_err"   >:::(List.map (fun f -> f>::test_does_err f)   (safe_readdir "input/do_err"    ".loch"));
      "dont_pass">:::(List.map (fun f -> f>::test_doesnt_run f) (safe_readdir "input/dont_pass" ".loch"));
      "dont_err" >:::(List.map (fun f -> f>::test_doesnt_err f) (safe_readdir "input/dont_err"  ".loch"))
    ]
