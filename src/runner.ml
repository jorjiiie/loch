...
let compile_string_to_string ?(no_builtins = false) (name : string)
...
let compile_file_to_string ?(no_builtins = false) (name : string)
...
let run_asm (asm_string : string) (out : string)
    (runner : string -> string list -> string -> (string, string) result) args
    (std_input : string) =
...
let test_run_anf ?(args = []) ?(std_input = "") program_anf outfile expected
    ?(cmp = ( = )) test_ctxt =
  let full_outfile = "output/" ^ outfile in
  let result = run_anf program_anf full_outfile run_no_vg args std_input in
  assert_equal (Ok (expected ^ "\n")) result ~cmp ~printer:result_printer

let test_run_valgrind ?(no_builtins = false) ?(args = []) ?(std_input = "")
    program_str outfile expected ?(cmp = ( = )) test_ctxt =
  let full_outfile = "output/" ^ outfile in
  let result =
    try
      let program = parse_string outfile program_str in
...
let test_err ?(no_builtins = false) ?(args = []) ?(std_input = "") program_str
    outfile errmsg ?(vg = false) test_ctxt =
  let full_outfile = "output/" ^ outfile in
...
let test_does_run filename test_ctxt =
  ...
  let progfile = sprintf "input/do_pass/%s.loch" filename in
  ...
  let output =
    if Sys.file_exists outfile then chomp (string_of_file outfile) else ""
  in
...
let test_does_err filename test_ctxt =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/do_err/%s.loch" filename in
  ...
  let args = parse_args argsfile opts in
  let err =
    if Sys.file_exists errfile then chomp (string_of_file errfile) else ""
  in
  let input = if Sys.file_exists infile then string_of_file infile else "" in
...
let test_doesnt_run filename test_ctxt =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/dont_pass/%s.loch" filename in
...
let input_file_test_suite () =
  ...
  >::: [
         ...
         >::: List.map
                ...
                (safe_readdir "input/do_pass" ".loch");
         "do_err"
         >::: List.map
                (fun f -> f >:: test_does_err f)
                (safe_readdir "input/do_err" ".loch");
         "dont_pass"
         >::: List.map
                (fun f -> f >:: test_doesnt_run f)
                (safe_readdir "input/dont_pass" ".loch");
         "dont_err"
         >::: List.map
                (fun f -> f >:: test_doesnt_err f)
                (safe_readdir "input/dont_err" ".loch");
       ]
